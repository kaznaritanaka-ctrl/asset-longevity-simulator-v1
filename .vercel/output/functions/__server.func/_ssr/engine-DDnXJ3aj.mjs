import { a as makePositiveDefinite, c as percentile, d as formatAge, i as gaussian, l as portfolioMoments, m as formatPct, o as mulberry32, p as formatManYen, r as cholesky, s as normalizeWeights } from "./routes-DA5D-Rb-.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/engine-DDnXJ3aj.js
...
function narrate(s) {
	...
	if (s.role === "median") {
		...
		if (s.ruined) parts.push(`この中央付近の経路でも${formatAge(s.ruinAge)}で条件に触れている。`);
	...
	if (!s.ruined) parts.push(`この乱数では破綻条件に触れなかった。残った経路のうち、いちばん細った一本を拾っている。${formatAge(s.endAge)}時点の残高は${formatManYen(s.terminal)}。`);
	else if (s.ruinAge <= s.fireAge) parts.push(`FIRE（${formatAge(s.fireAge)}）の前、${formatAge(s.ruinAge)}で資産が尽きた。積立期のまま相場に負けた経路。`);
	...
	if (s.ruined && s.preRuin5yAvg != null && s.preRuin5yAvg > 0 && s.worstYear > -.25) parts.push(`破綻直前5年の平均リターンは${signedPct(s.preRuin5yAvg)}。最後は大暴落というより、残った元本が支出に足りなくなった。`);
...
function pickRuinStory(args) {
	...
	let worst = 0;
	let worstW = Infinity;
	for (let i = 0; i < trials; i++) if (ruined[i]) ruinedIdx.push(i);
	else {
