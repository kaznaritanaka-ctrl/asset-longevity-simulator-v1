...
export function SourceCard({ embedded = false }: { embedded?: boolean }) {
  ...
  const detail = (
    <>
      <p className="text-xs leading-relaxed text-fg-muted">円建て長期実データ。{win.note}</p>
  ...
  if (embedded) {
    return (
      <details className="group rounded-lg border border-border bg-bg-sunken/45 px-3 py-2.5">
  ...
  return (
    <SectionCard title="使用するデータの選択" collapsible defaultOpen={false} preview={selector}>
      {detail}
    </SectionCard>
  );
}
