...
export function RuinEditor({ embedded = false }: { embedded?: boolean }) {
  ...
  const content = (
    <>
      <p className="mb-3 text-xs leading-relaxed text-fg-subtle">
        どれか一つに該当した経路を破綻とみなす（論理和）。初期値は「期間中に資産が0円以下」。100歳時点の残額で判定したい場合はチェックポイントを足す。
      </p>
  ...
  return (
    <SectionCard title="破綻条件の設定" titleNowrap action={addControl}>
      {content}
    </SectionCard>
  );
}
