    ...
    <article className="min-w-0 rounded-xl bg-surface p-4 shadow-[var(--shadow-border)] sm:p-5 md:p-6">
      <details className="group">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-fg-muted">
          この計算機は、資産形成期と取り崩し期を別の配分・別のキャッシュフローとして扱い、資産クラスごとの期待リターンとリスク、クラス間の相関を乗せてモンテカルロ経路を描く。資産枯渇は「資産がゼロ」に限らず、自分で置いた条件で判定する。
        </p>
        ...
        <ol className="mt-6 grid gap-5 md:grid-cols-2">
          ...
          <Note n="2" title="資産クラスのリターンとリスク">
            既定値は JST Macrohistory（株式・国債・手形）と World Bank 金価格を、日本CPIと円ドルで実質円建てにしたもの。算術平均と年率σをクラスごとに置き、相関付き正規乱数（コレスキー）で年次リターンを振る。年1回リバランス。出典ウィンドウ（最長／戦後／変動相場制）で差し替えられる。
          </Note>
          <Note n="3" title="ユーザー定義の資産枯渇">
            期間中の閾値割れ、指定年齢での残額、支出のN年分を下回る、の三つ。複数条件は論理和。例:「100歳時点で資産ゼロなら資産枯渇」はチェックポイント条件として足す。
          </Note>
          <Note n="4" title="経路の数え方">
            毎年、年初にキャッシュフローを適用し、その後リターンを乗せる。取崩期は手取り支出を保つため、売却額 = 不足分 / (1−税率)。負の資産は0に切り上げ、条件に触れれば資産枯渇フラグを立てて経路は続ける。資産枯渇した経路から無作為に一本、中央の帯にいちばん近い一本、をそれぞれ拾って語る。
          </Note>
