  ...
  return (
    <div className="min-w-0">
      <div
        ...
        data-track="1"
        className={cn(
          "relative h-11 cursor-pointer touch-none select-none",
          compact && "hidden",
        )}
      ...
      <div className={cn("mt-1 flex justify-between text-xs text-fg-muted", compact && "hidden")}>
        <span>{AGE_MIN}</span>
        <span>{AGE_MAX}</span>
      </div>
      ...
      >
        <AgeField compact={compact} label="現在" value={now} onCommit={(v) => apply("now", v)} />
        <AgeField compact={compact} label="FIRE" value={fire} onCommit={(v) => apply("fire", v)} />
        <AgeField
          compact={compact}
          label={"シミュレーション\n終了年齢"}
          value={end}
      ...
      {error || validationIssues.length > 0 ? (
        <div className="mt-2 rounded-lg bg-ruin-soft px-3 py-2 text-xs leading-relaxed text-ruin">
  ...
  return (
    <label
      className={cn(
        ...
        compact ? "w-full" : wide ? "w-[6.75rem] shrink-0" : "w-14 shrink-0",
      )}
    >
...
function ThumbHandle({
  ...
  return (
    ...
    >
      ...
      />
      <span className="pointer-events-none absolute -top-4 whitespace-nowrap text-xs font-medium tabular-nums text-fg">
        {formatAge(age)}
      </span>
    </button>
  );
