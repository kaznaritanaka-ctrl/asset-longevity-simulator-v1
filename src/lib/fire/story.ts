...
function narrate(s: {
  ...
  if (s.role === "median") {
    ...
    if (s.ruined) {
      parts.push(`この中央付近のシナリオでも${formatAge(s.ruinAge)}で条件に触れている。`);
    }
  ...
  if (!s.ruined) {
    parts.push(
      `この乱数では資産枯渇条件に触れなかった。残ったシナリオのうち、いちばん細った一本を拾っている。${formatAge(s.endAge)}時点の残高は${formatManYen(s.terminal)}。`,
    );
  } else if (s.ruinAge <= s.fireAge) {
    parts.push(
      `FIRE（${formatAge(s.fireAge)}）の前、${formatAge(s.ruinAge)}で資産が尽きた。積立期のまま相場に負けたシナリオ。`,
    );
  ...
  if (s.ruined && s.preRuin5yAvg != null && s.preRuin5yAvg > 0 && s.worstYear > -0.25) {
    parts.push(
      `資産枯渇直前5年の平均リターンは${signedPct(s.preRuin5yAvg)}。最後は大暴落というより、残った元本が支出に足りなくなった。`,
    );
  } else if (
    s.worstAge != null &&
    s.worstYear <= -0.22 &&
