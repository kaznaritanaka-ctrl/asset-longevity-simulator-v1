import { createDefaultPlan, matchingPhasePresetId, PRESETS } from "./defaults";
  ...
  if (statsMatchCatalog(plan)) {
    ...
    if (accumPreset && withdrawPreset) {
      if (accumPreset === withdrawPreset) {
        ...
        out.p = `${PRESET_CODE[accumPreset] ?? accumPreset}:${PRESET_CODE[withdrawPreset] ?? withdrawPreset}`;
      }
